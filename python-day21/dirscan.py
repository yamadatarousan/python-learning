"""
Day21: ミニCLIツール（大きいファイルTOP N）— CLI構成整理

狙い（このDayでやること）：
- CLIの入口(main.py)と、実装本体(dirscan.py)を分離する
- 依存方向を整理する（テストは dirscan.py を import して実行する）
- 「設定解決」「実行（走査/計算）」「出力（stdout/ファイル/POST）」を関数で分ける
  → どこがI/Oで、どこが純粋計算かが見えるようにする

設定の優先順位：
  CLI > env（OS環境変数 / --env-file） > config（JSON）

使い方（CLIは main.py 経由で実行）：
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ここでは「CLIとして受け取る項目」を列挙するだけに留め、
    env/configの優先順位や補完は別関数で行う。
    """
...
def parse_provided_options(argv: list[str] | None) -> set[str]:
    """
    どのオプションがCLIで明示されたかを判定する。

    目的：
    - config/envが「既定値」を埋めるのはOK
