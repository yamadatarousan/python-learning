"""
Day11: ミニCLIツール（ディレクトリ走査）— Day10の続き（最小サイズフィルタ）

Day11の追加点：
- --min-size BYTES を指定すると、
  指定サイズ未満のエントリは count / total / top から除外される。
"""
...
def scan_directory(
        root: Path, verbose: bool, mode: str, min_size: int,top_n: int
    ) -> tuple[int, int, list[Entry] | None]:
    """
    Day7/8のscanをDay9で拡張：
    - Entry一覧を作る
    - count/totalを計算する
    - 必要なら top N も計算して返す
    """
    entries = iter_entries(root, mode=mode, min_size=min_size, verbose=verbose)
