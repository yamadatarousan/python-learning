"""
Day14: ミニCLIツール（ディレクトリ走査）— loggingでstderrログを統一（Day13の続き）
...
- ログは print(stderr) ではなく logging に集約し、stderrに出す（JSON(stdout)を汚さない）
...
def iter_entries(root: Path, mode: str, min_size: int, logger: logging.Logger) -> list[Entry]:
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("scan start: root=%s mode=%s min_size=%d", root, args.mode, args.min_size)
