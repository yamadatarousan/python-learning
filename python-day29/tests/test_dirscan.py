"""
Day24: dirscan と toolkit の役割分担に合わせてテストも整理した版。

狙い：
- toolkit に移した共通関数（parse_bool / load_env_file / get_env / parse_provided_options）は toolkit を直接テストする
- dirscan 側のテストは「dirscan 固有の仕様」に寄せる
"""
...
def test_parse_bool_truthy_and_falsey() -> None:
    ...
    assert toolkit.parse_bool("1") is True
    assert toolkit.parse_bool("true") is True
    assert toolkit.parse_bool("YES") is True
    assert toolkit.parse_bool("on") is True

    assert toolkit.parse_bool("0") is False
    assert toolkit.parse_bool("false") is False
    assert toolkit.parse_bool("No") is False
    assert toolkit.parse_bool("off") is False
...
def test_load_env_file_parses_key_value_and_ignores_comments(tmp_path: Path) -> None:
    ...
    env = toolkit.load_env_file(env_path, logger)
...
def test_get_env_prefers_env_file_over_os_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert toolkit.get_env("DIRSCAN_TOP", env_file) == "2"
...
def test_apply_env_respects_cli_overrides_and_directory_from_cli(tmp_path: Path) -> None:
    ...
    provided = toolkit.parse_provided_options(["/from/cli", "--top", "3"])
    logger = toolkit.setup_logger("test", False)
...
def test_apply_config_only_fills_when_not_provided(tmp_path: Path) -> None:
    ...
    provided = toolkit.parse_provided_options(["--top", "3"])
    logger = toolkit.setup_logger("test", False)
...
def test_main_writes_out_file_when_env_file_sets_json_and_out(tmp_path: Path) -> None:
    ...
    # - DIRSCAN_OUT があれば、そのパスに payload を書き込む（toolkit.write_json_file 経由）
