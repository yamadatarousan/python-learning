"""
Day9: ミニCLIツール（ディレクトリ走査）— Day8の続き（Top N 追加）
...
Day8で「何を数えるか」を --mode として仕様化した。
Day9ではさらに「サイズが大きい順の上位N件」を表示するために --top を追加する。
...
【使い方】
    ...
    python main.py [directory] --top 10 --human
    python main.py [directory] --mode file --top 10   # top表示はこれが一番分かりやすい

【注意（仕様）】
- mode=file : 通常ファイルのみ（一般的な“ファイル”の期待に近い）
- mode=all  : ディレクトリ以外は全部（ソケット等も count に入る）
  ※ソケット等は st_size が 0 のことが多いので、--top の結果が薄くなりがち。
"""
