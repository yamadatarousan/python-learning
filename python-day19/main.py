...
def parse_bool(value: str) -> bool:
    ...
    if v in {"0", "false", "no", "n", "off"}:
        return False
    # 変な値は「空でなければTrue」扱いにしておく（ただし利用側で注意）
    return bool(v)
...
def apply_env(
    ...
    # directory（位置引数）：CLIで位置引数が指定されているなら env では上書きしない
    ...
    if "--post" not in provided:
        ...
        if v:
            args.post = v
    if "--out" not in provided and args.out is None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # CLIで位置引数(directory)が渡されたかどうか（config適用後だと判別できないので先に確保）
    directory_from_cli = args.directory is not None

    # Day18: どのオプションがCLIで明示されたか（config/envが上書きしてよいかの判断）
    provided = parse_provided_options(argv)

    # Day19: env-file を読む（OS環境変数より優先度は低い）
    env_file: dict[str, str] = {}
