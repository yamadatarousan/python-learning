"""
Day15: ミニCLIツール（大きいファイルTOP N）— ミニ課題（Day14の続き）

このプログラムは、指定ディレクトリ以下を再帰的に走査して、
「サイズが大きい順の TOP N」を表示/JSON出力するCLIツール。

Day15のポイント（ミニ課題）：
- “大きいファイルTOP N” に主目的を寄せて、表示と仕様を整理する
- 走査（I/O）と集計（計算）を分ける
- entries は list を作らず generator（yield）で流し、TOP N はストリーミングで保持する
- loggingでstderrログを統一（stdoutのJSONを汚さない）
